...
- A reply may transfer `0..=POST_REPLY_MAX_POINTS` points, with a default
  ...
  to. Transfers to one's own post are allowed by default and can be rejected
  with `POST_REPLY_ALLOW_SELF_POINTS=false`. Any amount greater than the
  sender's current balance is rejected.
