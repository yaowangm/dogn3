...
| Read user directory, including email addresses | Denied | Administrator only (`level >= 10`) | `/api/users` requires both an administrative session identity and a current administrative `user_info.level`. |
...
endpoint rechecks current stored administrator level before returning
