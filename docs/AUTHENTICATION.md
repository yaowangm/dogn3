...
| Add root post | Denied | Allowed | Allowed | Allowed | Require same-origin-fetch header; assign the authenticated user as author; maintain derived board/user counts and invalidate portal cache. |
| Update post content or image | Denied | Own posts only | Own posts only | Any post | Require same-origin-fetch header; prohibit author/tree/link changes through the editor; validate uploaded image formats and configured size; compress uploads above 500 KB below the stored-size threshold; maintain affected derived counts and invalidate portal cache. |
...
| Reply to post | Denied | Any visible post | Any visible post | Any visible post | Require same-origin-fetch header; server fixes reply type to normal; maintain tree order, root reply metadata, and derived counts transactionally. |
