...
| Update post content | Denied | Own root posts only unless used as signature | Own root posts only unless used as signature | Any post | Require same-origin-fetch header; enforce configured subject/content limits and a valid content format; prohibit author/tree/link/image changes through the editor; non-admin root updates preserve the existing post type, administrator root updates may change it, and non-root updates keep post type normal; any post appearing in `sign_log` is locked against non-admin edits; maintain affected derived counts and invalidate portal cache. |
