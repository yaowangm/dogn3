...
| Soft-delete post | Denied | Denied | Board-master boards only | Any post | Require same-origin-fetch header; set only the selected post to `state = 2`; preserve its stored row; refresh visible board/author statistics and invalidate portal cache. |
